...
- persistent cache configuration is read from both the Rspack 1 (`experiments.cache`) and Rspack 2 (top-level `cache`) locations; a legacy `experiments.cache` value is automatically applied to `cache` under Rspack 2 (with a warning), preventing silent loss of caching
