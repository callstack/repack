...
Newest Node versions support stripping TypeScript types out of the box. 
...
Consider the following example for Rspack configuration:
```ts title="rspack.config.ts"
import { defineRspackConfig } from '@callstack/repack';
import { RepackPlugin } from '@callstack/repack';

export default defineRspackConfig((env) => {
