...
The output-related configuration options have been removed from `RepackPlugin`. The following properties: `bundleFilename`, `sourceMapFilename`, `assetsPath` are now controlled exclusively through their respective CLI flags - [see bundle command documentation](/docs/commands/bundle). 

Remove these options from your plugin configurations since they are not needed anymore:
