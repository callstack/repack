...
Re.Pack provides first-class support for [React Native Reanimated](https://docs.swmansion.com/react-native-reanimated/) through a dedicated plugin that simplifies integration.
...
For Reanimated 4 and above, the worklet transformation is handled by the `react-native-worklets/plugin` babel plugin. The `ReanimatedPlugin` is used primarily to silence warnings.
...
If you're using webpack with `babel-loader`, you don't need the `ReanimatedPlugin`. Simply add the Reanimated babel plugin to your `babel.config.js` as shown above.
