...
With persitent caching enabled, you can expect your hot startups to be more than 6x faster:

| App | Without Cache - Cold Startup | With Cache - Hot Startup | Delta |
|-----|--------------|------------|-------------|
| Super App Showcase Host (Single Platform) | 2s | 0.3s | -85% |
| Super App Showcase Host(Both Platforms) | 3.4s | 0.45s | -87% |
| New Expensify App (Single Platform) | 10.5s | 1.4s | -87% |
| New Expensify App (Both Platforms) | 18.1s | 2.3s | -87% |

> Note: Single platform builds mean that bulds were run  `npx react-native start --platform ios` or `npx react-native start --platform android`, while both platforms builds are done with just `npx react-native start`.
